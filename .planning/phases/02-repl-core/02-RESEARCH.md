...
function createPrintCapture(maxPerCall, maxTotal) {
  ...
  function formatValue(value) {
    ...
    if (Array.isArray(value) && value.length > 5) {
      const preview = value.slice(0, 2).map(v => JSON.stringify(v)).join(', ');

      return 'Array(' + value.length + ') [' + preview + ', ... +' + (value.length - 2) + ' more]';
    }
  ...
  function print(...args) {
    ...
    if (totalChars + text.length > maxTotal) {
      text = text.slice(0, maxTotal - totalChars) + '... [truncated, ' + maxTotal + ' char limit]';
    }
...
const BUILTIN_GLOBALS = new Set([
  'workspace', 'projects', 'deps', 'dependents',
  'read', 'files', 'search', 'nx',
...
export function loadConfig(pluginRoot, workspaceRoot) {
  ...
  try {
    const raw = readFileSync(join(workspaceRoot, '.claude', 'lz-nx.rlm.config.json'), 'utf8');
...
function createSearch(workspaceRoot) {
  return function search(pattern, paths) {
    ...
    if (lines.length > 50) {
      return lines.slice(0, 50).join('\n') + '\n... [' + lines.length + ' total, showing first 50]';
    }
